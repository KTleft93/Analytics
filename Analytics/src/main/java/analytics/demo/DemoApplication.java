...
import analytics.demo.domain.*;
import analytics.demo.repository.*;
...
public class DemoApplication {
	...
	CommandLineRunner command2(UserRepository userRepo) {
		...
		return args -> {
			User user = new User("tyler", "kean", "kean@kean.edu", passwordEncoder.encode("Xy123456"), roles);
			userRepo.save(user);
